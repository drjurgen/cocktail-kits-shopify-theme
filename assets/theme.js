...
slate.a11y = {
	...
	trapFocus: function (options) {
		var focusableElements = Array.from(
			options.container.querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex^="-"])')
	...
	_onFocusIn: function (evt) {
		if (evt.target !== this.state.lastFocusable && evt.target !== this.state.firstFocusable) return;
		...
		xhr.onreadystatechange = function () {
			...
			if (xhr.status === 417) {
				try {
					...
					onError(
						new InvalidParameterError(xhr.status, invalidParameterJson.message, invalidParameterJson.description)
					);
				} catch (error) {
			...
			if (xhr.status === 422) {
				try {
					...
					onError(
						new ExpectationFailedError(xhr.status, expectationFailedJson.message, expectationFailedJson.description)
					);
				} catch (error) {
					onError(new JsonParseError(xhr.status));
				}
...
this.Shopify.theme.PredictiveSearchComponent = (function (PredictiveSearch) {
	...
	PredictiveSearch =
		PredictiveSearch && PredictiveSearch.hasOwnProperty("default") ? PredictiveSearch["default"] : PredictiveSearch;
	...
	function PredictiveSearchComponent(config) {
		...
		this.predictiveSearch = new PredictiveSearch(
			config.PredictiveSearchAPIConfig ? config.PredictiveSearchAPIConfig : DEFAULT_PREDICTIVE_SEARCH_API_CONFIG,
			this.searchUrl
		);
...
theme.Drawers = (function () {
	...
	Drawer.prototype.close = function () {
		...
		document.activeElement.dispatchEvent(new CustomEvent("blur", { bubbles: true, cancelable: true }));
...
theme.MobileNav = (function () {
	...
	function closeMobileNav() {
		...
		cache.mobileNavContainer.addEventListener("transitionend", mobileNavRemoveTrapFocus, { once: true });
	...
	function goToSubnav(target) {
		var targetMenu = target
			? document.querySelector('.mobile-nav__dropdown[data-parent="' + target + '"]')
			: cache.mobileNav;
		...
		cache.mobileNavContainer.addEventListener("transitionend", trapMobileNavFocus, { once: true });
	...
	var handleWindowScroll = theme.Helpers.debounce(function () {
		...
		for (var key in videos) {
			if (videos.hasOwnProperty(key)) {
				...
				var condition =
					videoWrapper.getBoundingClientRect().top + window.pageYOffset + videoWrapper.offsetHeight * 0.75 <
						window.pageYOffset ||
					videoWrapper.getBoundingClientRect().top + window.pageYOffset + videoWrapper.offsetHeight * 0.25 >
...
theme.SearchResultsTemplate = (function () {
	...
	function renderProduct(product, index, resultsCount) {
		return [
			...
			'<span class="visually-hidden">' + getNumberOfResultsString(index + 1, resultsCount) + "</span>",
			"</div>",
			"</a>",
			"</li>",
		].join("");
	...
	function renderProductDetails(product) {
		return [
			'<dl class="predictive-search-item__details price' +
				...
				(!product.isPriceVaries && product.isCompareVaries ? " price--compare-price-hidden" : "") +
				'">',
...
theme.Cart = (function () {
	...
	function Cart(container) {
		...
		this.cartRoutes = JSON.parse(document.querySelector("[data-cart-routes]").innerHTML);
	...
	Cart.prototype = Object.assign({}, Cart.prototype, {
		...
		_createCart: function (state) {
			...
			this.container.querySelector(selectors.cartSubtotal).innerHTML = theme.Currency.formatMoney(
				state.total_price,
		...
		_setItemRemove: function (item, title) {
			item
				...
				.setAttribute("aria-label", theme.strings.removeLabel.replace("[product]", title));
		},
				...
				function (property) {
					...
					if (property[0].indexOf("/uploads/") === -1) {
						...
						propertyElement.querySelector(selectors.cartItemPropertyValue).innerHTML =
							'<a href="' + property[1] + '"> ' + property[1].split("/").pop() + "</a>";
					}
		...
		_createUnitPrice: function (unitPrice, unitPriceMeasurement) {
			...
			var unitPriceBaseUnit =
				(unitPriceMeasurement.reference_value !== 1 ? unitPriceMeasurement.reference_value : "") +
				unitPriceMeasurement.reference_unit;

			unitPriceGroup.querySelector(selectors.unitPriceBaseUnit).textContent = unitPriceBaseUnit;

			unitPriceGroup.querySelector(selectors.unitPrice).innerHTML = theme.Currency.formatMoney(
				unitPrice,
				theme.moneyFormat
			);
		...
		_createItemDiscountList: function (item) {
			return item.line_level_discount_allocations.map(
				function (discount) {
					...
					discountNode.querySelector(selectors.cartItemDiscountAmount).innerHTML = theme.Currency.formatMoney(
						discount.amount,
						theme.moneyFormat
					);
		...
		_formatRemoveMessage: function (lineItem) {
			...
			return theme.strings.removedItemMessage
				...
				.replace("[link]", "<a " + 'href="' + url + '" class="text-link text-link--accent">' + title + "</a>");
		},
...
theme.Maps = (function () {
	...
	window.gm_authFailure = function () {
		...
		document
			...
			.insertAdjacentHTML("afterend", '<div class="' + classes.errorMsg + '">' + theme.strings.authError + "</div>");
	};
	...
	function Product(container) {
		...
		this.settings.zoomEnabled =
			this.imageZoomWrapper.length > 0
				? this.imageZoomWrapper[0].classList.contains(this.classes.jsZoomEnabled)
				: false;
	...
	Product.prototype = Object.assign({}, Product.prototype, {
		...
		_setupCartPopup: function (item) {
			...
			this.cartPopupQuantityLabel =
				this.cartPopupQuantityLabel || document.querySelector(this.selectors.cartPopupQuantityLabel);
			...
			this.cartPopupImagePlaceholder =
				this.cartPopupImagePlaceholder || document.querySelector(this.selectors.cartPopupImagePlaceholder);
		...
		_setCartPopupPlaceholder: function (imageUrl) {
			this.cartPopupImageWrapper =
				this.cartPopupImageWrapper || document.querySelector(this.selectors.cartPopupImageWrapper);
		...
		_setCartPopupProductDetails: function (product_has_only_default_variant, options, properties, selling_plan_name) {
			this.cartPopupProductDetails =
				this.cartPopupProductDetails || document.querySelector(this.selectors.cartPopupProductDetails);
		...
		_getSellingPlanHTML: function (selling_plan_name) {
			var sellingPlanHTML =
				'<li class="product-details__item product-details__item--property">' + selling_plan_name + "</li>";
		...
		_setCartQuantity: function (quantity) {
			this.cartPopupCartQuantity =
				this.cartPopupCartQuantity || document.querySelector(this.selectors.cartPopupCartQuantity);
			var ariaLabel;
...
theme.ProductRecommendations = (function () {
	function ProductRecommendations(container) {
		...
		var recommendationsSectionUrl =
			baseUrl + "?section_id=product-recommendations&product_id=" + productId + "&limit=4";
	...
	StoreAvailability.prototype = Object.assign({}, StoreAvailability.prototype, {
		updateContent: function (variantId) {
			var variantSectionUrl =
				this.container.dataset.baseUrl + "/variants/" + variantId + "/?section_id=store-availability";
			var self = this;
